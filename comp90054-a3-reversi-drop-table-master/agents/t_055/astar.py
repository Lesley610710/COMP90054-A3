...
class myAgent(Agent):
    ...
    def SelectAction(self,actions,game_state):
        ...
        #PQ is not empty
        while not (myPQ.isEmpty()) and (time.time() - start_time < 0.9):
            count += 1
            state, action_list, score = myPQ.pop()
            next_actions = self.game_rules.getLegalActions(state, self.id)

            if (not state in visited) or score > best_g.get(state):
                visited.add(state)
                best_g[state] = score
                #end of game or time greater than 0.96
                if (self.GameEnd(state) and self.WinLose(state)) or (time.time() - start_time) > 0.96:
                    curr_score = self.CalScore(state, self.id)
                ...
                for action in next_actions:
                    ...
                    cu_score = self.CalScore(next_state, self.id)
                    newN = (next_state, next_list, cu_score)
                    myPQ.push(newN, -(cu_score + self.heuristic(next_state)))
        if(changed == False):
            best_Q = -INFINITY
    ...
    def heuristic(self, state):
        ...
        #print(features)
        return 0.02 * h1 + (-0.02) * h2 + 1.2 * h3 + (-0.9) * h4 + (-1.5) * h5 + 0.9 * h6 + (-1 * h7) + 0.65 * h8 + 0.25 * h9 + (-0.4 * h10)  + 1.1 * h11
